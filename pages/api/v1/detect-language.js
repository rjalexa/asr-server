 ...
 *   post:
 *     summary: Audio Language Detection
 *     description: Detect the language of audio files using Whisper
 *     tags:
 *       - Language Detection
 *     security:
 *       - ApiKeyAuth: []
 ...
 *                 type: string
 *                 format: binary
 *                 description: Audio file for language detection (max 50MB)
 *             required:
 *               - audio_file
 *     responses:
 *       200:
 ...
 *       400:
 *         description: Bad request (invalid file, etc.)
 *         content:
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/Error'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const url = `${whisperApiUrl}/detect-language?${queryParams}`
    
    console.log(`[Language Detection v1 API] Forwarding request to: ${url}`)
    ...
    console.error('[Language Detection v1 API] Error:', error)
